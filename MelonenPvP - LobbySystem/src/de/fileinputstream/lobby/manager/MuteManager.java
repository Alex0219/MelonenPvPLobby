...
public class MuteManager {
    ...
    public static void mute(String UUID, String Playername, String Reason, long seconds, String Dauer, String operator)
    {
        ...
        Main.mysql.update("INSERT INTO Mutes (Spielername, UUID, Ende, Grund, Dauer, Muter) VALUES ('" + Player + "','" + UUID + "','" + end + "','" + Reason + "','" + Dauer + "','" + operator + "')");
        for (Player all : Bukkit.getOnlinePlayers()) {
            if (all.hasPermission("ban.notify")) {
                all.sendMessage("§cSystem §7● §7Der Spieler §e" + Playername + " §7wurde von §e" + getMuter(UUID) + " §7mit dem Grund §e" + Reason + "§7gemutet.");
            }
        }
    }
    ...
    {
        Main.mysql.update("DELETE FROM Mutes WHERE UUID='" + UUID + "'");
    }

    public static boolean isMuted(String UUID)
    {
        ResultSet rs = Main.mysql.query("SELECT Ende FROM Mutes WHERE UUID='" + UUID + "'");
        try
        {
            return rs.next();
        }
    ...
    {
        ResultSet rs = Main.mysql.query("SELECT * FROM Mutes WHERE UUID='" + UUID + "'");
        try
        {
            if (rs.next()) {
    ...
    {
        ResultSet rs = Main.mysql.query("SELECT Dauer FROM Mutes WHERE UUID='" + UUID + "'");
        try
        {
            if (rs.next()) {
    ...
    public static List<String> getMutedPlayers()
    {
        ...
        ResultSet rs = Main.mysql.query("SELECT * FROM Mutes");
        try
        {
            while (rs.next()) {
